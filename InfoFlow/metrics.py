# collection of scripts to evaluate the metrics.


# =============================================================================
#   Evaluates multiclass mAP and IoU 
#
...
def eval_bbox_mAP_and_IoU(predictfolder, gtfolder, resultsfolder, MINOVERLAP=0.25):

    """ expects the results per image frame output into specific folders. 
    ...
    print('checking input folders for evaluating bbox detection')
    print('predictfolder path: ', predictfolder)
    print('groundtruthfolder path: ', gtfolder)
    print('metric_results_folder path: ', resultsfolder)
    ...
    # if os.path.exists(results_files_path): # if it exists already, clean the directory!. 
    #     # reset the results directory
    #     shutil.rmtree(results_files_path)
    if not os.path.exists(results_files_path):
        os.makedirs(results_files_path) # remake! 
    ...
    with open(os.path.join(results_files_path, "results.txt"), 'w') as results_file:
        ...
        for class_index, class_name in enumerate(gt_classes):
            ...
            """
            Assign predictions to predicted objects
            """
    ...
    # #    jsonFileName='/output/metrics.json' 
    # else:
    #     jsonFileName='/output/metrics.json' 
    ...
    jsonFileName = os.path.join(resultsfolder, 'mAP_IoU_results.json')
    
    #from pathlib import Path
    #fileName=Path(jsonFileName)
    #
